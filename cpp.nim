...
proc parseDefBody(p: var Parser, m: var TMacro, params: seq[string]) =
  m.body = @[]
...
proc parseDef(p: var Parser, m: var TMacro; hasParams: bool) =
  m.name = p.tok.s
  getTok(p)
...
proc isIncludeGuard(p: var TParser): bool =
  var guard = p.tok.s
  skipLine(p)
  if p.tok.xkind == pxDirective and p.tok.s == "define":
