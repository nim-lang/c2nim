type
  ...
  rcutils_allocator_t* {.importc: "rcutils_allocator_t",
                         header: "rcl_allocator.h", bycopy.} = object ##
                              ##
    ...
    deallocate* {.importc: "deallocate".}: proc (pointer: pointer;
        state: pointer) ##  deallocate: Deallocate previously allocated memory, mimicking free().
                        ##   more lines
    reallocate* {.importc: "reallocate".}: proc (pointer: pointer;
        size: csize_t; state: pointer): pointer ##  reallocate: Also takes the `state` pointer.
    ...
    reallocate2* {.importc: "reallocate2".}: proc (pointer: pointer;
        size: csize_t; state: pointer): pointer ##  reallocate2: Also takes the `state` pointer.
